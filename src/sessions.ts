...
export class SessionManager {
  ...
  formatList(sessions: SessionData[]): string {
    ...
    for (const s of sessions) {
      ...
      const summary = s.summary || (typeof firstContent === 'string' ? firstContent.slice(0, 50) : '(empty)');
  ...
  private getSessionPath(id: string): string {
    // SECURITY: Validate id against a strict allowlist to prevent path
    // traversal (e.g. `../../etc/passwd`). Session ids are generated by
    // us (nanoid-style) and contain only [A-Za-z0-9_-].
